...
public class Controller extends Thread
{
    ...
    //----------------------------------------------------------
    private synchronized boolean selectionCheck(int selection)
    {
        boolean result = true;
        switch(userType)
        {
    ...
    public synchronized void addNewOrderItem(int orderID)
    {
        boolean done = false;
        int addItemID = 0;
    ...
    private void addNewMenuItem()
    {
        ...
        while(!done)
        {
            ...
            newType = 0;
            printMessageToView("Enter item type(1:Grocery 2:Electronics 3:BakeryItem 4:Cosmetics):");
            key = cView.userInput();
            while(newType == 0)
            {
                try
            ...
            switch(newType)
            {
                case MenuItem.Grocery:
                    printMessageToView("New item type:Grocery");
                    break;
                case MenuItem.Electronics:
                    printMessageToView("New item type:Electronics");
                    break;
                case MenuItem.BakeryItem:
                    printMessageToView("New item type:BakeryItem");
                    break;
                case MenuItem.Cosmetics:
                    printMessageToView("New item type:Cosmetics");
                    break;
            }   
        ...
        while(!done)
        {
            ...
            try
            {
                switch(inputNumber)
                {
                    ...
                    break;
                    case 3: //edit type
                        printMessageToView("Input new type(1:Grocery 2:Electronics 3:BakeryItem 4:Cosmetics):");
                        key = cView.userInput();
